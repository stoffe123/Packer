...
void fuzzProfile(packProfile* profile, packProfile best) {
	...
	profile->superSlimSeqlenMinLimit3 = doFuzz(profile->superSlimSeqlenMinLimit3, best.superSlimSeqlenMinLimit3, 0, 7000);
	profile->seqlenMinLimit3 = doFuzz(profile->seqlenMinLimit3, best.seqlenMinLimit3, 0, 7000);
...
void blockpack_onefile() {
	...
	while (true) {
			...
			int times = (int)(37000000.0 / (float)size_org);

			times = (times / 5) + 3;
			printf("\n TRYING EXT=%ls TIMES=%d", ext, times);
